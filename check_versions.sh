#!/bin/bash
# check_versions.sh - Check for new commits in Beanstalkd
...
read_current_versions() {
    if [ -f "$VERSIONS_FILE" ]; then
        CURRENT_COMMIT=$(grep -A3 '"beanstalkd"' "$VERSIONS_FILE" | grep '"commit"' | sed 's/.*"commit": *"//;s/".*//')
        CURRENT_VERSION=$(grep -A1 '"beanstalkd"' "$VERSIONS_FILE" | grep '"version"' | sed 's/.*"version": *"//;s/".*//')
    else
        CURRENT_COMMIT=""
        CURRENT_VERSION=""
    fi
...
# Main execution
echo "=== Beanstalkd Version Check Script ==="
echo "Mode: $MODE"
